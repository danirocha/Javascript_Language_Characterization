package main;

import java.io.BufferedReader;
import java.io.BufferedWriter;
import java.io.File;
import java.io.FileInputStream;
import java.io.FileWriter;
import java.io.IOException;
import java.io.InputStreamReader;
import java.util.ArrayList;
import java.util.List;

public class GeradorCSVFuncao {
	
	int nan = 0;
	private String nomeBiblioteca = "";
	String nomeFuncao = "";
	double loc = 0;
	double var = 0;
	double numChamFunc = 0;
	double halstead = 0;
	double cc = 0;
	double mi = 0;
	double numFuncFilhas = 0;
	private List<Double> esprimaValores = new ArrayList<Double>();
	private final String[] esprima = {"AssignmentExpression",
			"ArrayExpression",
			"BlockStatement",
			"BinaryExpression",
			"BreakStatement",
			"CallExpression",
			"CatchClause",
			"ConditionalExpression",
			"ContinueStatement",
			"DoWhileStatement",
			"DebuggerStatement",
			"EmptyStatement",
			"ExpressionStatement",
			"ForStatement",
			"ForInStatement",
			"FunctionDeclaration",
			"FunctionExpression",
			"Identifier",
			"IfStatement",
			"Literal",
			"LabeledStatement",
			"LogicalExpression",
			"MemberExpression",
			"NewExpression",
			"ObjectExpression",
			"Property",
			"ReturnStatement",
			"SequenceExpression",
			"SwitchStatement",
			"SwitchCase",
			"ThisExpression",
			"ThrowStatement",
			"TryStatement",
			"UnaryExpression",
			"UpdateExpression",
			"VariableDeclaration",
			"VariableDeclarator",
			"WhileStatement",
			"WithStatement",
			"ArrayPattern",
			"ObjectPattern",
			"ArrowFunctionExpression",
			"ClassExpression",
			"TaggedTemplateExpression",
			"Super", "YieldExpression", "AssignmentPattern",
			"RestElement", "MethodDefinition",
			"TemplateElement", "TemplateLiteral", "SpreadElement", "ForOfStatement"};
	private final String[] bibliotecasErro = {"calendario",
			"generator-silverstripe-module",
			"street",
			"jaydata-dynamic-metadata",
			"react-native-macos",
			"cubekit-ioc",
			"i18n-generator",
			"laxar-testing",
			"studyjs",
			"i18next-conv",
			"bibtex-parse-js",
			"html-select",
			"i18n-tag-examples-react",
			"logule",
			"currentscript",
			"md5-jkmyers",
			"resize-observer-polyfill",
			"uservoice-sso",
			"v8-inspector",
			"egg-core",
			"f.lux-react",
			"pnode",
			"fxjs",
			"jformatter",
			"provide-state",
			"xtuple-ice-cream",
			"get-blueprint",
			"k-browserchannel",
			"react-native-assets",
			"express-file-store",
			"fbmessenger",
			"npm-which",
			"partial.js",
			"web-game-framework",
			"generator-d3-module",
			"aurelia-route-recognizer",
			"superagent",
			"authx",
			"xmark",
			"is-git-clean",
			"yomo",
			"autosize",
			"everyauth-express3",
			"nobook-render-factory",
			"react-timeago",
			"stormbox",
			"battleship-search",
			"friedjuju",
			"hash-file",
			"n8-server",
			"react-native-text-input",
			"endothermic",
			"xpkg",
			"react-lazy-cache",
			"shortcut.js",
			"conf2p",
			"es2015-i18n-tag",
			"redux-json-api",
			"stylelint",
			"blizzardry",
			"i18next-scanner",
			"react-redux-i18n",
			"auth0-web-header",
			"aurelia-http-client",
			"neutrinoscript",
			"d8",
			"digger-redis",
			"pnode-store",
			"agejs",
			"babel-plugin-transform-adana",
			"ifile",
			"perfectapi",
			"dora-plugin-atool-build",
			"i18njs",
			"overseer-monitor",
			"react-dropzone-s3-uploader",
			"shelloid",
			"npm-user-packages",
			"read-pkg",
			"bower-installer",
			"cxla",
			"interpret-gcode",
			"jquery-expire",
			"site-password",
			"everywhere",
			"gwt-api-exporter",
			"appium-ios-driver",
			"react-native-status-bar-size",
			"three-obj",
			"babel-plugin-i18n-id-hashing",
			"generator-misha",
			"forte-api",
			"postcss-devtools",
			"react-native-storage",
			"tv4",
			"fallback-custom-scheme",
			"find-up",
			"react-bootstrap-v4",
			"navbar-native",
			"languages4translatewiki",
			"react-native-mobx",
			"general-store-crud",
			"mobx-ajv-form",
			"redux-falcor",
			"moment-component",
			"uikit",
			"bosco",
			"ng-forward",
			"aurelia-binding",
			"firebase-transactions",
			"tripleo-ui",
			"js-tokens",
			"react-gateway",
			"i18next-smart-bucket-backend",
			"react-router",
			"rn-packager",
			"buddy",
			"statsd-jut-backend",
			"ember-source",
			"koa-i18n",
			"binocular-api",
			"babel-plugin-css-modules-react",
			"framify",
			"react-native-x5",
			"excalibur",
			"load-json-file",
			"s3-webpack-plugin",
			"kad-boilerplate",
			"country-data-package",
			"eslint-config-i-am-meticulous",
			"micromatch",
			"babelfish-plus",
			"react-anime",
			"browserstack-runner-tehshrike",
			"galileo-io",
			"react-native-starter-kit",
			"decaffeinate-parser",
			"minio",
			"aurelia-webpack-plugin",
			"js-simple-plurals",
			"nativescript-liveedit",
			"react-native-bugsnag",
			"itako-text-transformer-dictionary",
			"login-text",
			"react-native-media-kit",
			"jwerty-globals-fixed",
			"o-yi",
			"amd-optimize",
			"pa11y-ci",
			"appium-xcuitest-driver",
			"iot-gateway-kit-misc",
			"react-native-navigation-buttons",
			"requisite",
			"fso",
			"mmmm",
			"react-intl-translations-manager",
			"react-toggle-switch",
			"jellymarker",
			"mixpanel-cli",
			"rn-amapview",
			"torrent-live",
			"is-up",
			"tenon-node",
			"mxd-heimdall",
			"datasift-trace",
			"ied",
			"clima-app",
			"prey",
			"koa-i18n-2",
			"@gearsandwiresapi-views-upload-s3",
			"appium-ios-simulator",
			"css-modules-require-hook",
			"gulp-cordova-preference",
			"appium-uiauto",
			"walker-sample",
			"cordova-minify-v2",
			"ls-x",
			"npm-keyword",
			"react-native-simple-stepper",
			"wdio-mocha-framework",
			"viewport-list",
			"smart-restart",
			"point2js",
			"node-canvas-text",
			"uneval",
			"penne",
			"select2",
			"wk-postmessenger",
			"generator-redux-app",
			"current-executing-script",
			"d2",
			"falcor-local-datasource",
			"inject-loader",
			"loopback-connector-ibmi",
			"graceful-copy",
			"bentham",
			"finnish-business-ids",
			"github-repositories",
			"iscroll-react",
			"thinknode",
			"gradle-to-js",
			"@m59reload-client",
			"jgallery",
			"x-git-grep",
			"aurelia-kendoui-bridge",
			"react-facebook",
			"v8-runtime-functions",
			"react-native",
			"react-redux-uuid",
			"react-smart-forms",
			"reuse-promise",
			"connect-js-c3",
			"strict-errors",
			"edx-custom-a11y-rules",
			"i18n-extract",
			"q-require",
			"react-styleguide-generator",
			"@covisintcui-idm-b2x",
			"ip2pwebos",
			"memsql-statsd",
			"jqueryp",
			"multy",
			"xtuple",
			"formsy-react-components",
			"hapi-serve-s3",
			"tellij",
			"kaoscript",
			"dd-webpack-service",
			"node-isimulator",
			"react-native-graphs",
			"react-native-custom-segmented-control",
			"redux-contract",
			"c3",
			"aurelia-dialog",
			"xtuple-jscommunicator",
			"jsxquery",
			"cacheman_v2",
			"cluetip",
			"babel-plugin-i18n-tag-translate",
			"koa-redis-ratelimit",
			"uncontrollable",
			"mocker-data-generator",
			"redux-i18n",
			"itero",
			"feathers-configuration",
			"m8",
			"j2m",
			"module-deps-diskless",
			"opensource-kit",
			"any-translate",
			"cadence",
			"filesize",
			"lumagl",
			"ted-crushinator-helpers",
			"generator-m",
			"immu",
			"@staakyvoila",
			"musepm-s3",
			"string-mutator",
			"peeracle",
			"dataminer",
			"i18next-node-locize-backend",
			"mobx-collection",
			"@vtexapi",
			"react-event-listener",
			"itc",
			"ios-app-utils",
			"webscreensaver",
			"gulp-i18next-conv",
			"metadata-js",
			"react-native-firestack",
			"@cypresscore-reporter",
			"jsss",
			"mobx--awesome-connect",
			"snoowrap",
			"tiNanta",
			"react-formagic",
			"react-native-autocomplete-input",
			"epub-zipper",
			"grunt-set-app-mode",
			"instantsearch.js",
			"react-native-carousel-control",
			"air-amap",
			"imageviewer",
			"wtwui",
			"react-i18n-universal",
			"localizr",
			"redux-form",
			"turntable-js",
			"cleverstack-cli",
			"hasha",
			"proletariat",
			"storejs",
			"principessa",
			"process-exists",
			"ripple-lib",
			"verifalia",
			"audio-type",
			"code.svg",
			"universal-react-boilerplate",
			"react-native-shadow",
			"flux",
			"appium-ios-log",
			"consistent-env",
			"mr",
			"v8-natives",
			"markdown-to-jsx",
			"node-mirror",
			"react-d3-svg",
			"normalize-newline-cli",
			"o2.date",
			"selenium-assistant",
			"slush-y",
			"react-map-gl",
			"u-wave-source-youtube",
			"canopy-node-client",
			"data-line-expansion-i2c",
			"notifyjs",
			"xcod",
			"custom-drag",
			"node-p4-swarm",
			"jetrunner",
			"p-break",
			"event-target",
			"react-helmet",
			"webpack",
			"xtuple-morpheus",
			"blockbuilder-search",
			"o2.amd",
			"nouislider",
			"statsd-cloudwatch-backend",
			"think-socket-uws",
			"yolk",
			"react-motion",
			"is-port-reachable",
			"s8fleet",
			"m-components",
			"package-json",
			"jquery.fullscreen",
			"i-react-utils",
			"opn",
			"orbit-core",
			"watchbot-progress",
			"jsonit",
			"dotenv-extended",
			"craftml",
			"mozilla-download",
			"date.format",
			"mdns-js",
			"reactmvc",
			"material-swagger-ui",
			"react-timeseries-charts",
			"react-toggle-button",
			"f.lux",
			"generator-kickstarting",
			"draft-app",
			"grunt-tx",
			"node-gc-viewer",
			"atsd-statsd-backend",
			"fdev",
			"rxjs-serial-subscription",
			"browserfs",
			"execa",
			"vue-i18n-manager",
			"gw2e-account-value",
			"u-wave-source-soundcloud",
			"microformat-shiv",
			"crawlkit-runner-htmlcs",
			"react-native-photo-viewer",
			"jimp",
			"y9-neptune",
			"pad-stream",
			"react-angular",
			"u-wave-source-dailymotion",
			"aurelia-mask",
			"express-locale",
			"graphql",
			"path-type",
			"files-filters",
			"scrutinize",
			"gigs",
			"grunt-web-development",
			"loc-i18next",
			"generator-latex-with-markdown",
			"haro",
			"node-enocean-button",
			"react-p2",
			"kizzy",
			"gw2e-recipe-calculation",
			"vast.js",
			"github-change-remote-file",
			"gw2e-item-attributes",
			"jquery-steps",
			"keigai",
			"movie-trailer",
			"holmes.js",
			"gw2e-recipe-nesting",
			"rekord",
			"testdouble",
			"toffee",
			"react-native-osd-datetimepicker",
			"router5",
			"gw2e-scraping",
			"nca-cli",
			"react-native-dropdownalert",
			"sshpk",
			"tweetcat",
			"lsofi",
			"rog3r",
			"webl10n",
			"git-remote-origin-url",
			"react-native-loading-w",
			"jquery-navToSelect",
			"mi18n",
			"crawlkit-runner-accessibility-developer-tools",
			"eslint-plugin-jsx-a11y",
			"if-stream",
			"iM880-serial-comm",
			"gw2e-account-statistics",
			"cherrytree",
			"webcoach",
			"react-native-smart-loading-spinner-overlay",
			"ujs",
			"npm-name-cli",
			"peace",
			"@robinnnnnreact-atv-img",
			"react-native-google-analytics-bridge",
			"type-r",
			"universal-redux-boilerplate",
			"react-native-swipe-out",
			"jquery-extends",
			"quagga",
			"aurelia-i18n",
			"will-it-optimize",
			"react-m",
			"react-octicon",
			"eslint-import-resolver-webpack",
			"osx-battery-utils",
			"pnotify",
			"rev-file",
			"@rhorse-react",
			"updeep",
			"basap",
			"mandrillctl",
			"jdataview",
			"react-email-blueprints",
			"react-physics",
			"circular-migration-plot",
			"boxen-notify",
			"react-native-maps-cluster",
			"gcode-parser",
			"r-swipeable",
			"math-2d",
			"supersphincs",
			"bluebird",
			"node-opcua",
			"node-req",
			"generator-express",
			"cookiejs",
			"delay",
			"jsdom",
			"signalr-tabex",
			"pixi-animationloop",
			"suman",
			"fn-args",
			"jquery-widget-compiler",
			"freebase_geo",
			"generator-a3",
			"maskloader",
			"eslint-config-a-standard",
			"fluorine-lib",
			"pixi-audio",
			"r26r-supervisor",
			"tls-certificate-transparency-log-alerter",
			"angular-recaptcha",
			"gulp-cmd-jst",
			"gulp-strip-code",
			"@rapi-client",
			"react-native-particle-system",
			"windows",
			"pyrite3dwebgl",
			"latest-version",
			"react-native-modal-android",
			"vvf",
			"immutability",
			"itunes-data",
			"peervisionary",
			"html-builder",
			"yellowleaf-cli",
			"redux-bug-reporter",
			"cachebust",
			"react-testutil-query",
			"bunnyjs",
			"adaptive-bezier-curve",
			"eu.numberfour.mangelhaft.reporter.xunit",
			"react-native-maps",
			"jquery.fancytree",
			"kwirk",
			"httpserver",
			"vinyl-read",
			"takipi-statsd",
			"chiffon",
			"toa",
			"jsql",
			"react-awesome-modal",
			"react-swipe",
			"vtex",
			"o2",
			"babel-preset-cordial",
			"@stamenareachart",
			"technique",
			"tubbs",
			"h264-live-player",
			"o-o",
			"alfred-playground",
			"react-native-transformable-image",
			"asset-mate",
			"mtimes-changed",
			"jquery-adaptText",
			"uspace-sdk-ui",
			"appium-youiengine-driver",
			"npm-name",
			"optimized",
			"expose-s3",
			"http-five-o",
			"koa-exception",
			"tree-exchange",
			"admin-mongo",
			"i18n-simple",
			"labjack-nodejs",
			"preprocessor",
			"react-native-analog-clock",
			"flight-control-tower",
			"react-native-responder-view",
			"unzip-response",
			"cloud-cache",
			"mobx-router",
			"node-c-analyzer",
			"blapy",
			"jquery-uix-multiselect",
			"event-drops",
			"myquery",
			"sand",
			"jsxlate",
			"react-native-modal-dropdown",
			"draft-js",
			"react-translate-extended",
			"idcheckio",
			"web-caddy",
			"httpinvoke",
			"react-native-item-cell",
			"ember-cli-htmlbars-l10n",
			"redbox-react",
			"angular-component-way-webpack-starter-kit",
			"react-autosizer",
			"canvas-webworker",
			"merge",
			"set-immediate-promise",
			"vectorz",
			"authorize-ios",
			"rss-o-bot",
			"d2-ui",
			"is-in-viewport",
			"webdriverio",
			"generator-ustack-ued",
			"opn-cli",
			"tt-editor",
			"allons-y-routes",
			"c-preprocessor",
			"generator-eslint",
			"itunes-checker",
			"ecstatic",
			"tman",
			"casperjs",
			"easier-react-native",
			"jquery-toggles",
			"generator-react-static",
			"xy",
			"hypernova",
			"react-tabs-redux",
			"jquery-component",
			"react-native-amap-view",
			"react-tag-input",
			"cncjs",
			"@r7kamuradraft-js-import-markdown",
			"brite",
			"smartface-c9",
			"ultra-repl",
			"objj-runtime",
			"ics",
			"d3-dtree",
			"steam-chat-bot",
			"nodetron",
			"fxos-simulators",
			"kaomoji",
			"npm-synonym",
			"l-react-bootstrap",
			"gherkin-ace",
			"react-simple-format",
			"phastly",
			"tunic",
			"nud3",
			"a-standard",
			"aurelia-fetch-client",
			"maga",
			"migrate-orm2",
			"rserve-js",
			"allons-y-web-helper",
			"radium",
			"simple-reverse-geocoder",
			"transifexjs",
			"hollaback",
			"i18n-light",
			"keo",
			"react-anything-sortable",
			"@cypresscore-runner",
			"react-onsenui",
			"node-cube",
			"r-modal",
			"rea11y",
			"nobraces",
			"react-native-notifications",
			"strong-service-install",
			"glslify",
			"pjs",
			"pouchdb-adapter-http",
			"rmc-list-view",
			"storybook-addon-a11y",
			"baconpancakeifier",
			"vinyl-file",
			"react-native-intl",
			"i18n-browser",
			"react-media",
			"react-intl-redux",
			"d3-state-visualizer",
			"appcache-manifest",
			"itako-text-reader-speech-synthesis",
			"access-sniff",
			"jbinary",
			"laxar",
			"pkg-dir",
			"leaflet-geosearch",
			"loggifiy",
			"rosemary-ui",
			"assure",
			"espresso",
			"@kualisimple-tree",
			"scan-a-r",
			"spotcheck",
			"website-popup",
			"fbemitter",
			"arabic-cypher",
			"react-setup",
			"strip-bom-stream",
			"hls-kit",
			"rdotjson",
			"ygor",
			"redux-little-router",
			"graphql-relay",
			"glob",
			"react-presents",
			"cbioportal-api-client",
			"react-fontawesome",
			"@rprivate",
			"file-bytes",
			"interlock",
			"seneca",
			"statsd-wavefront-backend",
			"broccoli-typescript-compiler",
			"react-ultra-date-picker",
			"iso-swiper",
			"yeoman-environment",
			"is-d",
			"mp3tagger",
			"r-table",
			"react-lang",
			"node-source-walk",
			"img2text",
			"nwmatcher",
			"@economistreact-i13n-omniture",
			"meteor-messageformat",
			"mr-angular-i18n",
			"luma.gl",
			"webui-boilerplate",
			"bardcode",
			"compilex",
			"private-grunt-contrib-uglify-wly",
			"info-rut",
			"mobious",
			"poopy.js",
			"rAppid.js-sprd",
			"g4",
			"passwd-user",
			"multireducer",
			"@fengyuanchenvalidator",
			"glm-js",
			"fela-plugin-lvha",
			"nxus-core",
			"react-i",
			"ebay-sdk",
			"bumps",
			"ember-cli-deploy-s3",
			"hyperform-l10n",
			"pathgl",
			"octokat",
			"react-i18nify",
			"venezuela",
			"google-music-sync",
			"pouchdb-replication-stream",
			"eRx-build",
			"postino",
			"react-native-simple-onboarding",
			"flash-b2g",
			"module-definition",
			"@nodert-win10windows.devices.i2c",
			"parse-server",
			"c9",
			"aurelia-loader-default",
			"f2etest-recorder",
			"ai-for-shogi-like-games",
			"r-arbol",
			"dn2a",
			"react-dnd-html5-backend",
			"@joeybakerwatchify",
			"figure.io",
			"n-gramify",
			"react-native-eventsource",
			"react-native-gifted-messenger",
			"v.js",
			"gulp-file-include",
			"attack",
			"fstream-ignore",
			"react-native-mapview",
			"plugged-in",
			"rxjs",
			"cl-n",
			"dropdown-react",
			"mfi",
			"id-geo",
			"info-command",
			"skid",
			"angular2-seed-advanced",
			"promise-thunk-retryify",
			"xcharts",
			"brake",
			"notie",
			"aframe-react",
			"anim8js-jquery",
			"@novemberbornas-i-preach",
			"pubnub-rickshaw-memory",
			"http-xak",
			"nutra",
			"redux-devtools-diff-monitor",
			"geo-proximity",
			"yarn",
			"axoid",
			"es6ify",
			"react-native-locale-service",
			"through2",
			"async-form",
			"css-matrix",
			"Meteor-Test-Installer",
			"port-pid",
			"fruit-loops",
			"int17",
			"promise-fn",
			"vue-y-table",
			"lenin",
			"chui",
			"browserify-ngannotate",
			"ember-sparkles",
			"@economistreact-i13n-piano",
			"vox-core-clr",
			"async-listener-x",
			"jailed-t",
			"refraction",
			"jwebdriver",
			"n-mock",
			"read-chunk",
			"fe-upload",
			"mobx-react",
			"eslint-test-generator",
			"grunt-stonejs-tools",
			"babel-jsxgettext",
			"generator-angular2-ts",
			"react-deploy-s3",
			"synchd",
			"utea",
			"convert-to-4chan-webm",
			"react-desktop",
			"redux-model-s",
			"source-notes",
			"tilestrata",
			"containor",
			"karma-rollup-plugin",
			"icreate",
			"adafruit-i2c-lcd",
			"generator-angular-mobile",
			"@rplatform",
			"telit-modem",
			"autowired-js",
			"dc.leaflet",
			"deployr-cli",
			"provisioning",
			"@rwidgets",
			"redux-ioc",
			"hibp",
			"datamaps",
			"insert-module-globals",
			"node-r3",
			"asciify",
			"react-nabu",
			"jquery-waterfall",
			"zip-to-s3",
			"nej-minimage",
			"scxml",
			"gulp-jasmine-browser",
			"microservicebus.node",
			"n7-react-weui",
			"staats",
			"hkpl-watchman",
			"react-color",
			"react-refetch",
			"browserify-xcharts",
			"jsonstream2a",
			"dc.graph",
			"get-gravatar",
			"trepo-ptree",
			"react-native-places-autocomplete",
			"redux-midi",
			"xet",
			"atool-l10n",
			"idb-request",
			"generator-h5-ng",
			"gitb",
			"react-native-mapzen-places-autocomplete",
			"redux-saga-router",
			"debugger.html",
			"hilary",
			"localer",
			"setflags",
			"osx-version",
			"tabard",
			"arborjs",
			"entitlements",
			"georedis",
			"pangyp",
			"weex-loader",
			"react-overflow-scrolling",
			"react-router-form",
			"deck.gl",
			"geometry-2d",
			"native-nao",
			"nodejs-dashboard",
			"compile-js-like-c",
			"flyweb-koa",
			"generator-lb",
			"iced-js",
			"sequelize-relay",
			"@rredux-state-archiver",
			"module-generator",
			"gw2e-requester",
			"scube",
			"product-version",
			"a1-redux-storybook",
			"react-routing-resolver",
			"steller",
			"wiiu-media-server",
			"hand-over",
			"mathbox",
			"react-translate-maker",
			"depict",
			"signalerjs-react",
			"gitinfo",
			"raptor-polyfill",
			"d3-block",
			"pkgcloud_v.1.3_hack",
			"reactstrap",
			"formsy-material-ui",
			"eslint-config-d3banking",
			"axoid-commons",
			"deploy-web-to-s3",
			"pizza-guy",
			"spar",
			"broccoli-persistent-filter",
			"total.js",
			"medium-editor-markdown",
			"observable-to-promise",
			"utils-plus",
			"jquery-asTooltip",
			"react-redux-observable",
			"translate-maker",
			"react-gettext-parser",
			"generator-nm-standard",
			"flowtypes",
			"mobiledoc-kit",
			"timecamp-v2-javascript-client",
			"detectrtc",
			"disrespect-caps",
			"wdio-cucumber-framework",
			"gitbook-start-github-rafadanipedro",
			"grunt-template-tmp",
			"list-directory-contents",
			"documentation",
			"detox-tools",
			"koa-test-stack",
			"moz-scope",
			"commonjs-jquery",
			"dreemgl",
			"react-redux-firebase",
			"xi4n",
			"delivr",
			"flux-capacitor-boot",
			"generator-speedseed",
			"toa-router",
			"traverse-directory",
			"xueditor",
			"exec-buffer",
			"@exoplayexobot-db-s3",
			"engine-x",
			"@typicodepegasus",
			"react-baobab",
			"dcrawler",
			"wdio-sync",
			"angular-i18n",
			"ij-rpc-client",
			"metascript",
			"react-native-adbannerview",
			"webpack-dev-server",
			"babel-plugin-transform-decorators-legacy",
			"stresser",
			"treed",
			"plumber-requirejs",
			"react-native-redux-routing",
			"shortcut-url",
			"gw2e-async-promises",
			"chanjet-gzq-h5",
			"help-me-test",
			"react-relay",
			"dynamo-client",
			"react14-pick",
			"bluebird-co",
			"fielddb",
			"koa2-dot",
			"plupload",
			"trifle",
			"thali",
			"voting-app-r282",
			"blur-js",
			"flapper-news",
			"s3sync-cli",
			"generator-scaffold",
			"jquery-asProgress",
			"ms-webpack",
			"parse-epub",
			"0x00-pl--avg-pack-to-ttf",
			"estore",
			"d3-react-sparkline",
			"aurelia-history",
			"modelo",
			"urlencoded-body-parser",
			"jquery-dynamicNumber",
			"react-nvd3",
			"bin-version-check",
			"jquery-poolparty",
			"react-dash",
			"react-gif",
			"react-native-bottom-toolbar",
			"izi-js-jquery",
			"parse-png",
			"fx-ports",
			"redeyed",
			"webpack-rtl-plugin",
			"pify",
			"aurelia-templating-binding",
			"webcat",
			"browserify-istanbul",
			"simple-i18n-text",
			"cordova-plugin-inapppurchase",
			"infinite-tree",
			"react-native-picker-m",
			"react-webpack-node",
			"v8-flags",
			"mady",
			"react-dropdown-w-react13",
			"generator-x",
			"d3.parcoords.js",
			"generator-react-redux-universal",
			"readfile",
			"react-native-router-native",
			"xm-el",
			"@christian-bromannbenchpress",
			"generator-mapapps",
			"jsorm-i18n",
			"react-native-key-value-store",
			"redux-form-uikit",
			"osx-version-cli",
			"react-computed-props",
			"react-stockcharts",
			"woobi",
			"globalize",
			"xgettext-regex",
			"shiitake",
			"worker-pouch",
			"dom-handler",
			"es6-geometry",
			"dead-package-finder",
			"p-try",
			"react-flexbox-grid",
			"generator-marionettor",
			"child-process-close",
			"node-i2cssh",
			"babel-plugin-transform-react-stateless-component-name",
			"cm",
			"react-native-picker-xg",
			"default-browser-id",
			"carrack",
			"dopple",
			"generator-sotojuan",
			"react-jsonschema-form",
			"react-select-plus",
			"generator-angular2-eggs",
			"react-native-desktop",
			"restart-hook",
			"s3-uploader-cli",
			"u-wave-web",
			"tmake",
			"react-async-actions",
			"qminer",
			"gitbar-lite",
			"jbart5-ng2",
			"skul",
			"bin-version",
			"nxus-s3-files",
			"draught",
			"react-native-timepicker",
			"jquery-i18next",
			"jss",
			"ms-files-gce",
			"react-custom-render",
			"to-fast-properties",
			"@codelennyjimp",
			"lightsword",
			"pixi-scene",
			"papp",
			"when",
			"mobx-utils",
			"p-reduce",
			"react-for-atom",
			"react-native-button-component",
			"tenon",
			"google-translate-api",
			"fontmetrics",
			"iterm-version",
			"react-commit-calendar",
			"react-compose",
			"awsops",
			"dora-plugin-simulator",
			"react-stripe-checkout",
			"reptar",
			"gispl.js",
			"grunt-prebuilder",
			"amd-driver-scripts",
			"react-intl-native",
			"react-native-collection",
			"v8-profiler",
			"mount-point",
			"react-progress-button",
			"react-svg-gauge-fork-custom",
			"eva.js",
			"nodemailer-base64-to-s3",
			"react-router-active-component",
			"immutable-angular",
			"meetup-trn-extractor",
			"x11",
			"rc-upload-s3",
			"seglir",
			"d2doc-dgeni-packages",
			"react-flux",
			"newrelic-api",
			"stonejs-tools",
			"aurelia-dependency-injection",
			"com.nrswolf.cordova.push-notifications-status",
			"jsdb-server",
			"spritesheet-js",
			"cstring",
			"executable",
			"gwatch",
			"common-tags",
			"jtipsy",
			"litescript",
			"lory.js",
			"h5-play",
			"jquery-asModal",
			"iaas",
			"jquery-migrate",
			"ph-cascade-selector",
			"nix-services",
			"limit-size-stream",
			"jmpress",
			"generator-ng6s",
			"sqlite4.js",
			"@offirmoi18n",
			"mobx-react-form",
			"njm",
			"ph-image-upload",
			"react-native-nmrangeslider-ios",
			"lithree.js",
			"media-query-facade",
			"react-echarts3",
			"translate-maker-loader",
			"aurelia-event-aggregator",
			"jstree",
			"koa-timeout-v2",
			"react-native-menu",
			"glazed",
			"itinify",
			"l-array-helpers",
			"gw2e-gw2api-client",
			"is-hireable",
			"jqoteplus",
			"r3-charts",
			"riverboat",
			"generator-sql-fullstack",
			"jsdoc3-parser",
			"jsonresume-theme-kwan",
			"woodman",
			"htfloml",
			"i2g",
			"blitline-s3",
			"epeg.js",
			"s3-fetch",
			"instrument-v8",
			"marko-v2",
			"react-menu-simple",
			"stl-viewer",
			"ethereum-web-token",
			"generator-n3dst4-package",
			"jQuery-QueryBuilder",
			"rapydscript-ng",
			"xblocks-core",
			"ytemplater",
			"babel-plugin-eslint-test-generator",
			"broadwayjs",
			"broccoli-es7-decorator-jshint-guards",
			"get-amd-module-type",
			"ccsk",
			"download.jqueryui.com",
			"serialize-form",
			"pp",
			"webdrivercss-custom-v4-compatible",
			"infinite-scroll-x",
			"stylelint_d",
			"core-dump",
			"lockfile",
			"react-toastr",
			"redux-infinite-scroll",
			"archive-type",
			"react-grid-layout",
			"rorybot",
			"simple-file-input",
			"aurelia-framework",
			"ph-selector",
			"aframe-extras",
			"ghost-axe",
			"minijasminenode-relaunchable",
			"gnap-theme-gnap-angular",
			"d3act-pie",
			"inquirer-menu",
			"ph-image-list",
			"rism",
			"check-es3-syntax",
			"dev-ip",
			"mobx-app",
			"paperclip3x",
			"rimraf",
			"dos",
			"react-gmaps",
			"express-g",
			"ngmap",
			"@gasbuddyswagger-ref-resolver",
			"plugnative-ionicv2-sidemenu",
			"angular2-humanize",
			"lococo",
			"cordova-plugin-upnp",
			"babel-plugin-contracts",
			"smart-house-broker",
			"@stephenbunchevent",
			"ember-frost-sort",
			"Grow.js",
			"update-v8",
			"react-currency-input",
			"react-native-quiltview",
			"aframe-ui",
			"mochawesome",
			"react-svg-inline",
			"controltower-api",
			"gw2e-gw2api-scraping",
			"nodeclipse",
			"react-admin-components",
			"redux-react-i18n",
			"generator-fangular",
			"koa-neo4j",
			"react-native-yandexmapkit",
			"generator-grunt-h5",
			"homemade",
			"isomorphic-flux-boilerplate",
			"microformat-node",
			"react-day-picker-v2-beta",
			"react-swipeable-views",
			"aws-s3-sync",
			"decko",
			"emoji-recovery",
			"sosumi",
			"skatejs",
			"webp2p",
			"o2.strip",
			"ramda-extra",
			"then-exec",
			"enzyme-to-json",
			"gcode-toolpath",
			"v8-changelog",
			"earthling",
			"pelcod-decoder",
			"asyncbox",
			"clean-copy",
			"cordova-plugin-xhr-blob-polyfill",
			"medium-editor-insert-plugin",
			"urijs",
			"ewdGateway",
			"ljswitchboard-modbus_map",
			"brucedown",
			"realm",
			"babel-plugin-transform-h-jsx",
			"genial",
			"e5r-dev",
			"garage11",
			"react-native-ios-charts",
			"stanly",
			"ember-cli-emscripten",
			"react-redux-transaction",
			"@yuheiyresponsive-image-map",
			"review-times",
			"react-native-banner-lite",
			"g9",
			"staticnook",
			"alt",
			"react-native-native-video-player",
			"screenshot-stream",
			"icloud-tabs",
			"nanoislands",
			"react-d3-ctx",
			"s3-site-sync",
			"h5-compiler-ejs",
			"redux-modules",
			"react-native-gallery",
			"eslint-config-screeps-neat-o",
			"npm-user",
			"react-icheck",
			"gcode-interpreter",
			"babel-preset-eslatest-node6",
			"y9-mars-service",
			"rx.wamp",
			"concurrency-logger",
			"nojquery",
			"vagrant-e2etesting-protractor",
			"generator-gnap-angular",
			"grimoirejs",
			"react-router-server",
			"trex-parsers",
			"jmd5s",
			"qtip2",
			"whoarewe",
			"protractor-multicapabilities-htmlreporter_v2",
			"redux-pagan",
			"loopback-developer",
			"appjs-linux-x64",
			"shellex",
			"babbler-js-material-ui",
			"e.router",
			"p3",
			"p5",
			"react-d3-histogram",
			"vue-class-component",
			"cldr.js",
			"declarative-promise",
			"grunt-skyscanner-i18n",
			"rn-flabel-textfield",
			"angular-es",
			"p-finally",
			"glisp",
			"mobx-input",
			"react-mobile-picker",
			"mframework",
			"@gandireact-translate",
			"koa-socket",
			"reapop",
			"terbit",
			"acorn-objj",
			"h5p",
			"graphy",
			"laravel-elixir-config",
			"jquery-depends-on",
			"generator-rest",
			"generator-web",
			"repanel",
			"osx-info",
			"giftpack-f2e-generator",
			"promise-fns",
			"browser-unpack",
			"xblocks",
			"mainline",
			"peermaps",
			"shuoshubao",
			"generator-strong-d3",
			"mkdirp",
			"bluetooth",
			"buffer-v6-polyfill",
			"try-hard",
			"valid-sift",
			"drawback",
			"lookerupper",
			"p-map",
			"s3url",
			"verb-nurbs",
			"grunt-s3-sync",
			"timepicker-ng",
			"angular-q-spread",
			"babel-plugin-transform-es2015-generator-return",
			"react-facebook-login-light",
			"epubjs",
			"p-tap",
			"github-user-contributions",
			"into-stream",
			"srv-cli",
			"defaultdict-proxy",
			"mongo-oplog",
			"node-aurora",
			"p-if",
			"immutable-ics",
			"react-d3-wrap",
			"redux-fireuser",
			"v8-debug",
			"x-error",
			"path-exists",
			"selectize",
			"weex-x",
			"yadr",
			"decorator-wrap",
			"angular-material",
			"mOxie",
			"request-html-charset-fix",
			"p-any",
			"restyle-lite",
			"p-memoize",
			"react-fluxible-i18n",
			"generator-webapp-impro",
			"p-some",
			"tesla-api",
			"preprocessor.js",
			"angular-selectize",
			"bfd-ui",
			"r2pipe",
			"objj-compiler",
			"react-three-renderer-html3d",
			"humanlize-spawn",
			"angular-ecmascript",
			"p-s",
			"aurelia-notification",
			"objit",
			"videojs-contrib-hls-s1",
			"mar-e-bici-bikes",
			"p-time",
			"generator-ullasoc",
			"mobx-react-devtools",
			"l10n-api",
			"http-host-proxy",
			"@kentcdoddsreact-stopwatch",
			"react-tabs-fork",
			"osx-app-version",
			"bugger-v8-client",
			"d3-svg-annotation",
			"p-limit",
			"hot-file-cache",
			"tito",
			"i-api",
			"broadway-player",
			"keystone-impatient",
			"fetch-api",
			"abstrax",
			"p-retry",
			"react-grecaptcha",
			"react-imageloader",
			"react-native-google-places",
			"video-controller",
			"deisdash",
			"p-whilst",
			"react-native-simple-markdown",
			"aurelia-templating-resources",
			"cross-json",
			"webpack-isomorphic-tools",
			"generator-oe-h5",
			"fxosbuilds",
			"j-unquote",
			"smart-bucket",
			"empfindung",
			"nodecast-js",
			"j-quote",
			"p-pipe",
			"koa-xtime",
			"linechart",
			"lock_and_cache",
			"gettext-loader",
			"rucksack-css",
			"gulp-license-check",
			"p-all",
			"riot-i18n",
			"generator-yawa",
			"my-proxy",
			"p-log",
			"p-debounce",
			"path-parser",
			"react-bootstrap-t2",
			"react-util",
			"store-s3-blobs",
			"trashss",
			"h5psd",
			"rainbow-gen",
			"winston-s3",
			"lazo.org",
			"p-props",
			"redux-remote",
			"xtuple-dashboard-anything",
			"generator-latex",
			"p-throttle",
			"hablar",
			"kalliope.runner",
			"marsdb-angular",
			"react",
			"react-native-geocoding",
			"abigail",
			"gh-user",
			"react-d3-charts",
			"p-filter",
			"@ptomasroosreact-native-multi-slider",
			"es6ify-with-subarg",
			"mobx-keys-store",
			"react-rangeslider",
			"automata-canvas",
			"plugin-iaas-passport-rafadanipedro",
			"tfl-busboy",
			"p-immediate",
			"get-stream",
			"mobx-translate",
			"p-timeout",
			"react-native-scrollable-tabs",
			"browserify-middleware",
			"@jnvhistory",
			"i18n-webpack",
			"iot-rest-api-server",
			"react-decoration",
			"generator-element",
			"p-catch-if",
			"ai",
			"balanced-node-v1.1",
			"friendly-errors-webpack-plugin",
			"react-native-MultiSlider",
			"wait-p",
			"angular-decorators",
			"imagemin-webp",
			"bitcoin-core",
			"yanatra",
			"dev-time-cli",
			"embedza",
			"rmc-listview",
			"babel-plugin-react-css-modules",
			"jqlouds",
			"p-settle",
			"find-babel-config",
			"seneca-web-adapter-koa2",
			"generator-jdi-component",
			"giu",
			"x-13arima-seats-bin",
			"p-wait-for",
			"goblinphysics",
			"browser-run",
			"compressor-g4",
			"fallwall",
			"p-race",
			"mdns-js2",
			"p-defer",
			"react-three-renderer",
			"seek-bzip",
			"rabbits-engine",
			"reactify2",
			"retryify",
			"x-flux",
			"gulp-tojson",
			"tinier",
			"authy-client",
			"jquery.jsonrpc",
			"makerjs",
			"p-each-series",
			"promisify-function",
			"react-toolbox",
			"a4",
			"dexie",
			"aws-mqtt-client",
			"graphql-compose-connection",
			"p-map-series",
			"PaasyMcPaasFace",
			"appium-safari-driver",
			"Gon",
			"p-locate",
			"redux-replicate",
			"rotten",
			"titon-toolkit",
			"sugar-js",
			"tls-certificate-transparency-log-checker",
			"material-ui-t",
			"p-times",
			"postcss-cssnext",
			"react-tabs",
			"jefri-modeler",
			"uni3d",
			"mx6",
			"cocos-utils",
			"mathoid",
			"react-native-fscalendar",
			"ibmi",
			"reactotron",
			"cocos2d",
			"mern-cli",
			"node-gyp",
			"p-flux",
			"react-native-icons",
			"dora",
			"generator-nm",
			"promethify",
			"rx-jquery",
			"p-waterfall",
			"data-path-store",
			"osx-quit",
			"ctt-reader",
			"extract-intl",
			"grpc-event-store",
			"p-queue",
			"react-x11",
			"generator-jdi",
			"iterm-colors",
			"osc-requests",
			"p-series",
			"terrain-obj",
			"brigadehub-admin-c4sf",
			"xtuple-field-dev-sample",
			"ch4t",
			"probe-image-size",
			"eslint-config-opent2t",
			"islider-h5",
			"bower-name",
			"es7-autocomplete",
			"hidden-files",
			"lx-scan",
			"jsdoc-api",
			"electrum-store",
			"npm-base-t",
			"p-forever",
			"protonmail-export",
			"kafkaesque",
			"objj-acorn",
			"p-min-delay",
			"react-native-instagram",
			"shex",
			"aurelia-logging",
			"webappengine",
			"react-g-analytics",
			"chinese-redux-auth",
			"binary-tree-d3",
			"@cyclecore",
			"uzh-courses-with-no-module",
			"axe-core",
			"kuba-bootstrap",
			"ethereumjs-devp2p",
			"x-mongoose-acl",
			"jqbuild",
			"postcss-custom-selectors",
			"solarnetwork-d3",
			"elementz-s3",
			"ekoa",
			"v1sdk",
			"canvas-designer",
			"hook.io-i18n",
			"locize",
			"biquad-coeffs-webaudio-v2",
			"generator-couchapp",
			"mako-js",
			"node-ninja",
			"cldrjs",
			"reactive-superglue",
			"@adludiogeometry",
			"react-native-lax1n-form",
			"routux",
			"simplifyify",
			"w3c-domcore-errors",
			"generator-kraken",
			"n3",
			"w3c-css",
			"webrtc-explorer",
			"react-native-globalize",
			"blue-button",
			"craziness",
			"mpay24-node",
			"cordova-plugin-billing",
			"github-token-user",
			"mongodb_s3_backup",
			"package-hash",
			"react-reader",
			"mert",
			"node-pngquant-native",
			"redbox-noreact",
			"redux-x-action",
			"swift-enum",
			"dataloader",
			"wreq",
			"karma-browserify",
			"respizer",
			"find-versions-cli",
			"ng-inject",
			"react-native-logger",
			"aurelia-task-queue",
			"cc.fovea.cordova.purchase",
			"jsss-compiler",
			"eventtarget",
			"infusion",
			"teen_process",
			"rdotswift",
			"react-select",
			"web-ext",
			"popper.js",
			"jquery-jsonrpc",
			"falcor-socket-datasource",
			"@jpjuni0rmantra-core",
			"react-native-google-cast",
			"6to5-loader",
			"activetick-addon",
			"etch",
			"vaco-components-library",
			"run-js",
			"webcomponents.js-v1",
			"ember-cli-fill-murray-e-karma",
			"uvCharts",
			"hyperkv",
			"i18n-tag-schema",
			"react-infinite",
			"osx-bluetooth",
			"tenon-cli",
			"chai-v8-optimisation",
			"icebreaker-msgpack",
			"x-browser",
			"box-di",
			"lokum",
			"certpem",
			"odejs",
			"onvif",
			"imsg",
			"write-json-file",
			"generator-launchpad",
			"wdio-jasmine-framework",
			"blynk-library",
			"slush-node-typescript2",
			"clusterpoint",
			"react-cookie-banner",
			"relpers",
			"draft-md",
			"react-spinner-children",
			"react-native-theme-manager",
			"tea-macro",
			"dreambox",
			"js-import-sort",
			"react-dapp-boilerplate",
			"redux-devshare",
			"u2j",
			"write-pkg",
			"zombify",
			"frame5",
			"generator-angular-eggs",
			"sauce-connect-launcher",
			"frisbee",
			"hypercloud",
			"egg",
			"thauto-worker",
			"angular-annotations",
			"dora-plugin-proxy",
			"metalsmith-react-templates",
			"object-invariant-test-helper",
			"presentation-push",
			"radic-cli",
			"find-config",
			"hpm-cli",
			"spindel",
			"aurelia-templating-router",
			"cyfin-account-lib",
			"eccrypto",
			"koa-better-error-handler",
			"quote-stream",
			"react-c3-component",
			"generator-webapp-rjs",
			"node-coolq",
			"textualization",
			"browser-sync-x",
			"webvr-dom",
			"jquery-asPieProgress",
			"itako-audio-reader-audio-element",
			"p-promise",
			"jquery-inputsearch",
			"eslint-config-auth0-base",
			"react-native-dates",
			"w3counter",
			"c0t",
			"inaturalist_api",
			"koa2-boilerplate",
			"react-native-nytphotoviewer",
			"sip.js-mnQf2Q2R",
			"fetch-http-client",
			"i18next-locize-backend",
			"package-info",
			"bicycle",
			"segmentedcontrol",
			"paye",
			"@kt3kpuncher",
			"teaspoon",
			"i11e-core",
			"neo4-js",
			"github-username",
			"tape-async",
			"jquery-enable",
			"moment",
			"nodeis",
			"readable-stream",
			"bundle-name",
			"clean-webpack-plugin",
			"q-tpl",
			"react-broadcast",
			"janimate",
			"mobx-state-tree",
			"ng2in1",
			"wrtc",
			"w3c-manifest-loader",
			"eg-tree-menu",
			"@webpack-blockswebpack",
			"nano-event-emitter",
			"pretty-bytes-cli",
			"app-path-cli",
			"relay-local-schema",
			"@sindresorhusdf",
			"cached",
			"react-quill",
			"react-with-modal",
			"linux-app-list",
			"nbind",
			"w-js",
			"bookshelf",
			"marko-migrate",
			"p-list",
			"@covisintcui-i18n",
			"decorangles",
			"fussutils",
			"react-resizable",
			"@sunny-grx-utils",
			"api-sheriff",
			"cssnano-cli",
			"githubapi.js",
			"jsonp-client",
			"cbfsss",
			"gulp-unassert",
			"remark-comment-blocks",
			"cloudsync",
			"d3-geo-projection",
			"infamous",
			"buildo-react-components",
			"react-relax",
			"stage-js",
			"redux-auth-wrapper",
			"babel-plugin-transform-inline-imports-commonjs",
			"react-storybook-addon-intl",
			"koa-generator",
			"redux-batch-enhancer",
			"sscd",
			"gun",
			"nfifo",
			"i18next",
			"postcss-selector-namespace",
			"react-native-nav",
			"sagemathcloud",
			"current-path",
			"react-native-firebase-analytics-bridge",
			"redux-responsive",
			"cdefs",
			"cezerin",
			"swig",
			"xit",
			"active-graph-record",
			"generator-sp",
			"inferno-src",
			"jhtml",
			"react-twzipcode",
			"three",
			"angular-html5",
			"compose-function",
			"ah-graphql-plugin",
			"f-utility",
			"babel-solo",
			"factory-girl",
			"globby",
			"react-router-es3",
			"react-planner",
			"k-esprima",
			"p5.js-svg",
			"pmvc",
			"rtcmulticonnection-v3",
			"ionic",
			"react-point",
			"bundle-id-cli",
			"f2o",
			"aurelia-router",
			"beame-sdk",
			"emoJiS-interpreter",
			"mixin-decorator",
			"n-space",
			"stubbify",
			"pivotaltracker",
			"react-router-nr",
			"skywalker-x",
			"nginx-redirect-writer-cli",
			"wfquery",
			"mz700-js",
			"package-json-to-readme",
			"synapse_pay",
			"travis-after-all",
			"grunt-dustjs-linkedin",
			"gulp-npm-test",
			"pickadate",
			"pirouette-toolchain-darwin-x64",
			"react-slide2d",
			"react-intl-es6",
			"varstruct-cstring",
			"react-chart-loading",
			"babel-plugin-rewire",
			"react-native-loading-spinner-overlay",
			"react-player",
			"mobx-react-router",
			"react-router-match-async",
			"react-sidebar",
			"react-vis",
			"statsd-http-interface",
			"asynchron",
			"bundle-name-cli",
			"jqcloud2",
			"tlang",
			"temp-write",
			"get-browsers",
			"zap-base-dom-element-transition",
			"cqwrap",
			"kongfig",
			"node-hammer",
			"winston-aws-cloudwatch",
			"gw2reset",
			"react-intl",
			"codegradxvmauthor",
			"js-include",
			"menuoptions",
			"react-router-fetch-data",
			"skylinkjs",
			"d3tooltip",
			"gerrit-f-button",
			"webpack-server-render-middleware",
			"corrode",
			"homematic-manager",
			"npool",
			"sagui",
			"n-resolve",
			"osx-brightness",
			"react-redux-webpack2-starterkit",
			"container-js",
			"transhand",
			"twgl.js",
			"jsnetworkx",
			"aurelia-logging-console",
			"n8iv",
			"react-bootstrap-table",
			"nly-react-input-range",
			"react-json2d",
			"promisen",
			"babel-plugin-bem",
			"paper",
			"toga-markdown",
			"storj-cli",
			"esp-js",
			"formsy-react",
			"react-native-check-box",
			"d3fc",
			"px2rem",
			"cp-file",
			"datachannel",
			"dnd-page-scroll",
			"gzip-size-cli",
			"injecorator",
			"jshint280",
			"osx-app",
			"generator-i-want-nice-ng-cordova",
			"k9bot",
			"parse-git-status",
			"persistent-c",
			"react-paginate",
			"babel-plugin-macros",
			"resocket",
			"unsplash-js",
			"veronica-x",
			"jquery-lifestream",
			"react-dates",
			"react-native-store",
			"box-content",
			"nad",
			"tanda",
			"tinyg",
			"foreground-child",
			"react-blocks",
			"shifty",
			"@rokt33race-builds",
			"dev-time",
			"loop54-js-demo",
			"react-image-depth",
			"react-fields",
			"react-native-photos-framework",
			"trianglify",
			"@ramitosxmldom",
			"awsmap",
			"q-connection",
			"aurelia-charts-c3",
			"requirejs-zhangxd6",
			"redux-polyglot",
			"lucid",
			"baconjs",
			"dio.js",
			"minerva",
			"unicode-2.1.2",
			"generator-javascript",
			"desk-ui",
			"react-widgets",
			"tp-flasher",
			"bin-check",
			"mobx-cache",
			"react-native-unlock",
			"whs",
			"gulp-espower",
			"iframely",
			"vue-i18n",
			"current-path-cli",
			"generator-k",
			"jspos",
			"chordify",
			"vue-smoothscroll-websites",
			"ember-cli-adapter-pattern",
			"fluorine-orchestra",
			"swagger-parser-x",
			"decofun",
			"grindstone",
			"d3-funnel",
			"async-update-props",
			"codegradxagent",
			"koa-graphql-next",
			"any-translate-adapter-babelfish",
			"better-react-spinkit",
			"generator-ua",
			"cinestrip",
			"generator-morf",
			"wanda",
			"generator-egzpo",
			"kube-watch",
			"react-native-nativemodule-example",
			"yjs",
			"n8-make",
			"fireball-docx",
			"battery-level",
			"lodash-decorators",
			"micons",
			"swarmbot",
			"xing-promise",
			"agregate",
			"aurelia-api",
			"cashew-js",
			"@vzvu3k6kdaichkr-client",
			"react-docgen",
			"snakeandladder",
			"generator-soa",
			"re-bulma",
			"tusboy",
			"d3-object-charts",
			"hook.io-fork",
			"angularjs-nvd3-directives",
			"generator-jade-rabbit",
			"paws.js",
			"generator-typo3",
			"jquery-sparkline",
			"md-standalone",
			"parse-copyright",
			"understudy-promise",
			"aurelia-typing-trigger",
			"react-native-toggle",
			"kindergarten",
			"react-native-navigation",
			"k-webgl",
			"node-red-contrib-readability",
			"jquery.event.pointertouch",
			"node-odata",
			"lumbur",
			"niconico",
			"promised-mongo",
			"react-native-google-places-autocomplete",
			"reactrangeslider",
			"atom-k",
			"scream",
			"ember-i18n-extended",
			"phaser",
			"react-native-webview-js-context",
			"framework7",
			"tango-chart",
			"ng-falcor",
			"zeroclipboard",
			"dora-plugin-weinre",
			"rfc2253",
			"g-music",
			"krampus",
			"kwik-e-mart",
			"postgraphql",
			"local-web-server",
			"react-native-mock",
			"react-i18next",
			"v2rx",
			"angular-annotation-decorator",
			"mute",
			"react-docer",
			"react-lazy-load-x",
			"react-pagan",
			"blear.ui.rich-editor",
			"react-native-simple-toast",
			"redux-devtools-dock-monitor",
			"cordova-plugin-local-notifications",
			"flatpickr",
			"json-schema-to-flow-type",
			"cordova-plugin-chromecastios",
			"buddha-bless-you",
			"dilite",
			"linear-algebra-js",
			"zerver-j",
			"react-side-effect",
			"apng2webp",
			"wordpos",
			"abigail-plugin-env",
			"couchdb-indexer",
			"cssnext",
			"lambda-update-alias",
			"react-data-components-bd2k",
			"react-html5video",
			"s3urls",
			"node-vitals",
			"web-gpio-react",
			"mapbox-gl",
			"patternfly-timeline",
			"phaser-examples-typescript",
			"mxd-auth-commands",
			"react-outside-event",
			"svg.js",
			"angular-async-await",
			"g-sheeter",
			"react-intl-rawformat",
			"class-private-method-decorator",
			"digitalocean-v2",
			"fux",
			"wpapi",
			"style-it",
			"i18n-yaml",
			"fast-admin",
			"jquery-powertip",
			"xena",
			"generator-aspnetcore",
			"fis3-prepackager-imweb-ques",
			"react-instagram-login",
			"jsonic",
			"grunt-ember-i18n-include",
			"vinyl-rw",
			"dark-mode",
			"qjs",
			"react-jquery-datatables",
			"react-native-simple-auth",
			"json-reactor",
			"@s2kagencyreact-dropzone",
			"cordova-plugin-local-notifications-pgb",
			"geo-tree",
			"p4js",
			"b-flat",
			"generator-desk-themes",
			"s10d-touchable",
			"jubilee",
			"rickshaw",
			"grunt-init-xblock",
			"patternlab-node",
			"victory-pie",
			"react-aars",
			"@tobiiproskylinkjs",
			"vm2",
			"iopa-ssdp",
			"react-native-ios-controllers",
			"locizify",
			"mithril-codemods",
			"babel-preset-env",
			"random-hash",
			"react-search-input",
			"norcal",
			"angular-decorators-b",
			"evaporate",
			"gulp-replace",
			"linq.js",
			"browserify-cache-api",
			"ae-to-json",
			"cola-api",
			"locator-lang",
			"stylestats",
			"sb-event-kit",
			"luhn-mod-n",
			"slate",
			"40xda",
			"api-flow",
			"tabletop-api",
			"tablo",
			"js-makedepend",
			"latest-push",
			"q-orm",
			"cerebral",
			"d3-financial-components",
			"s3-tus-store",
			"ncurl",
			"xtuple-linguist",
			"react-native-image-carousell",
			"rename-files-b",
			"split-guide",
			"brocode",
			"m2m-status",
			"react-native-facebook-login",
			"bplustree",
			"dool-build",
			"grunt-djangularjs-translate",
			"rapidgame",
			"ox",
			"react-native-filesystem",
			"add-asset-html-webpack-plugin",
			"algo-kmeans",
			"bindux",
			"xlsjs-node-4",
			"m3u8-parser",
			"crox-p",
			"get-stdin",
			"rich-functional-list",
			"dc",
			"osx-vol",
			"sidemenu",
			"cns",
			"gulp-docker-swellrt",
			"is-trademarked",
			"itunesfm",
			"broccoli-i18n-lazy-lookup",
			"pygmentize-bundled",
			"react-listbox",
			"skypager-devpack",
			"debounce-promise",
			"generator-m-ionic",
			"oauth-twitt",
			"react-alpha-beta",
			"cssnano",
			"victory-pie-native",
			"ng-mobx",
			"luminous-lightbox",
			"react-native-s3",
			"@dprattnode-k8s-client",
			"uioc",
			"neft-ios",
			"actionhero",
			"hacktabl-parser",
			"itako-audio-reader-audio-context",
			"react-d3-cloud",
			"route-node",
			"phx",
			"big-decimal",
			"s3-tail-stream",
			"vulp",
			"electron-text-substitutions",
			"oxygen-i18n",
			"redux-oidc",
			"jsxgettext",
			"@mean-expertloopback-sdk-builder",
			"react-native-splash-screen",
			"react-native-effects-view",
			"react-i18n-helper",
			"react-native-voice",
			"react-frau-intl",
			"vhtml",
			"slide2d-editor",
			"react-native-page-previewer",
			"react-tab-layout",
			"react-native-float-rating-view",
			"d3-react-graph-shapes",
			"re-natal",
			"react-native-pullview",
			"react-native-simple-picker",
			"react-native-segmented-android",
			"react-native-time-picker",
			"react-globe",
			"react-native-actionsheet",
			"react-native-webview2",
			"react-native-google-place-picker",
			"react-native-gesture-password",
			"react-native-android-modal",
			"react-native-toast-easy",
			"react-u5auth",
			"react-native-wechat-ios",
			"react-native-evernote",
			"redux-lang",
			"babylon",
			"react-native-multiple-choice",
			"react-native-callout",
			"@financial-timesn-section",
			"react-native-quick-look",
			"react-native-custom-action-sheet",
			"react-bubble-chart",
			"component-horizontal-stacked-bar",
			"d3-react-graph-basic",
			"react-native-share-sheet",
			"vdux-transition",
			"jail-monkey",
			"react-native-camera-roll-picker",
			"react-native-uuid-generator",
			"react-native-inapp-browser",
			"sparkbars",
			"flint-babylon",
			"react-native-videocore",
			"react-image-fallback",
			"react-native-select-input-ios",
			"upad-parse",
			"react-native-animated-tabs",
			"cpr-mask",
			"react-native-webviewhook",
			"react-countup",
			"rn-shimmer",
			"react-container-hoc",
			"jquery-builder",
			"react-native-beacons",
			"react-async-await",
			"vdux-position",
			"gentelella",
			"react-native-qrcode-view",
			"react-toggle-display",
			"react-native-floating-label-text-input",
			"cesium",
			"grunt-apps-c",
			"react-native-mbprogresshud",
			"react-native-audio-streaming",
			"unicode-7.0.0",
			"vdux-dropdown",
			"fullswipe",
			"react-native-progress-circular",
			"react-native-webbrowser",
			"unicode-8.0.0",
			"adminbsb-materialdesign",
			"react-card",
			"mathjax",
			"react-native-webbrowser-enhanced",
			"react-native-statusbar-alert",
			"react-native-pull",
			"react-native-nativebutton",
			"react-native-pagecontrol",
			"react-native-headers",
			"react-dnd-components",
			"react-native-simple-emoji-picker",
			"mobx-provide",
			"mljs",
			"react-i-cli",
			"react-m-carousel",
			"react-native-airplay",
			"f1-redux-utils",
			"react-native-picker-purejs",
			"unicode-9.0.0",
			"react-intl-proptypes",
			"render-shallow",
			"d3-react-graph-core",
			"react-native-universal-modal",
			"typescript",
			"generator-dictyweb",
			"react-native-easy-toast",
			"react-native-tabbar-navigator",
			"react-jupyter-display-area",
			"react-native-spotlight-search",
			"react-minimalist-portal",
			"react-native-sk-loader",
			"react-native-jelly-side-menu",
			"histoslider",

};
	
	float numFunc1 = 0;
	float numFunc2 = 0;
	
	public void geraCSV() throws IOException {
		//String caminho = "C:\\Users\\marcio.barros\\Desktop\\dist\\results";
		String caminho = "C:\\Users\\luisb\\Desktop\\novosTestes\\results";
		
		BufferedWriter bw = createCSV();
		BufferedWriter bwEsprima = createCSVEsprima();
		
		for(int i = 0; i < esprima.length; i++)
			esprimaValores.add(0.0);
		
		procuraArquivos(caminho, bw, bwEsprima);
		bw.close();
		bwEsprima.close();
		System.out.println(numFunc1);
		System.out.println(numFunc2);
	}
	
	private BufferedWriter createCSVEsprima() throws IOException {
		//String caminhoArquivoExportacao = "C:\\Users\\marcio.barros\\Desktop\\dist\\por_funcao_e_esprima.csv";
		String caminhoArquivoExportacao = "C:\\Users\\luisb\\Desktop\\novosTestes\\por_funcao_e_esprima.csv";
		
		BufferedWriter bw = new BufferedWriter(new FileWriter(caminhoArquivoExportacao));
		
		bw.write("nomeBiblioteca\t");
		
		for(int i = 0; i < esprima.length; i++) {
			if(i < esprima.length - 1)
				bw.write(esprima[i] + "\t");
			else
				bw.write(esprima[i] + "\n");
		}
		
		return bw;
	}
	
	private void procuraArquivos(String caminho, BufferedWriter bw, BufferedWriter bwEsprima) throws IOException {
		
		File diretorioLido = new File(caminho);
		
		File arquivos[] = diretorioLido.listFiles();
		
		for(int i = 0; i < arquivos.length; i++) {
			File arquivo = arquivos[i];
			
			if(arquivo.isDirectory())
				procuraArquivos(arquivo.getAbsolutePath(), bw, bwEsprima);
			
			else {				
				
				boolean achou = false;
				
				for (int j = 0; j < bibliotecasErro.length; j++) {
					String biblioteca = bibliotecasErro[j];
					
					if(biblioteca.compareToIgnoreCase(arquivo.getName().replace(" R.txt", "")) == 0) {
						achou = true; break;
					}
				}
				
				if(achou)
					continue;
				
				
				leArquivo(arquivo, bw, bwEsprima);
			}
		}
	}

	private void analisaArquivo(String linha, BufferedWriter bw, BufferedWriter bwEsprima) throws IOException {
		String[] dados = linha.split(";");
		
		if(dados[4].compareTo("NaN") == 0) {
			nan++;
		}
		
		nomeBiblioteca = dados[1];
		
		if(dados[2].compareTo("---") != 0) {
				
			if(nomeFuncao.compareTo(dados[2]) != 0)				
				nomeFuncao = dados[2];
			
			if(dados[3].compareTo("loc") == 0) {
				loc = Double.parseDouble(dados[4]);
			}
					
			else if(dados[3].compareTo("var") == 0) {
				var = Double.parseDouble(dados[4]);
			}
					
			else if(dados[3].compareTo("func") == 0) {
				numChamFunc = Double.parseDouble(dados[4]);
			}
			
			else if(dados[3].compareTo("dfunc") == 0) {
				numFuncFilhas = Double.parseDouble(dados[4]);
			}
									
			else if(dados[3].compareTo("cc") == 0) {
				cc = Double.parseDouble(dados[4]);
			}
					
			else if(dados[3].compareTo("halstead") == 0) {
				halstead = Double.parseDouble(dados[4]);
			}
					
			else if(dados[3].compareTo("mi") == 0) {
				mi = Double.parseDouble(dados[4]);
				
				escreveDados(bw);
							
				loc = 0;
				var = 0;
				numChamFunc = 0;
				numFuncFilhas = 0;
				cc = 0;
				halstead = 0.0;
				mi = 0.0;			
			}
			
			else {
				for(int j = 0; j < esprima.length; j++) {
					String noEsprima = esprima[j];
						
					if(noEsprima.compareTo(dados[3]) == 0){
						esprimaValores.set(j, Double.parseDouble(dados[4]));
						break;
					}
				}
				
				if(dados[3].compareTo("ExportNamedDeclaration") == 0)
				{
					escreveDadosEsprima(bwEsprima);
					for(int h = 0; h < esprima.length; h++)
						esprimaValores.set(h, 0.0);
				}
			}
		}
	}
	
	private void escreveDadosEsprima(BufferedWriter bw) throws IOException {
		
		bw.write(nomeBiblioteca + "\t");
		
		for(int i = 0; i < esprimaValores.size(); i++) {
			if(i < esprimaValores.size() - 1)
				bw.write(esprimaValores.get(i) + "\t");
			else
				bw.write(esprimaValores.get(i) + "\n");
		}
		
		numFunc2++;
	}

	private void escreveDados(BufferedWriter bw) throws IOException {
		bw.write(nomeBiblioteca + "\t" + loc + "\t" + var + "\t" + numChamFunc + "\t" + numFuncFilhas + "\t" + halstead + "\t" + cc + "\t" + mi + "\n");
		numFunc1++;
	}

	private void leArquivo(File arquivo, BufferedWriter bw, BufferedWriter bwEsprima) throws IOException {
	    BufferedReader in = new BufferedReader(new InputStreamReader(new FileInputStream(arquivo.getAbsolutePath()), "UTF8"));
	    
	    String linha;

		while ((linha = in.readLine()) != null) {
	    	
	    	if(linha.compareTo("caminho;biblioteca;nomeFuncao;analise;valor") != 0)
	    		analisaArquivo(linha, bw, bwEsprima);
	    }
	 
		in.close();
	}

	private BufferedWriter createCSV() throws IOException {
		//String caminhoArquivoExportacao = "C:\\Users\\marcio.barros\\Desktop\\dist\\por_funcao.csv";
		String caminhoArquivoExportacao = "C:\\Users\\luisb\\Desktop\\novosTestes\\por_funcao.csv";
		
		BufferedWriter bw = new BufferedWriter(new FileWriter(caminhoArquivoExportacao));
		bw.write("nomeBiblioteca\tloc\tnumVar\tnumChamFunc\tnumFuncFilhas\thalstead\tcc\tmi\n");
				
		return bw;
	}
}
